...
def add(
    ...
    """Add a card to db."""
    summary = " ".join(summary) if summary else None  # type: ignore
    with cards_db() as db:
        db.add_card(cards.Card(summary, owner, state="todo"))  # type: ignore
...
def update(
    ...
    with cards_db() as db:
        try:
            db.update_card(
                card_id,
                cards.Card(summary, owner, state=None),  # type: ignore
            )
        except cards.InvalidCardId:
            print(f"Error: Invalid card id {card_id}")
